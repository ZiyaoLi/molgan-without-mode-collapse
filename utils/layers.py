...
def masked_softmax(logits, masks):
    ...
    output = energy / tf.reduce_sum(energy, axis=-1, keep_dims=True)
    return output


def attn_head(queries, keys, values, adj, units=None, residual=False):
...
def gat_layer(inputs, units, n_heads, training, activation=None, dropout_rate=0.):
    ...
    output = tf.concat([attn_head(annotations, annotations, annotations, adj, units=units // n_heads)
