...
from utils.sparse_molecular_dataset import SparseMolecularDatasetWithRewards
...
def train_feed_dict(i, steps, epoch, epochs, min_epochs, model, optimizer, batch_dim):
    ...
    else:  # use conditional training in the second phase.
...
def eval_feed_dict(i, epochs, min_epochs, model, optimizer, batch_dim):
    phase = 'pretrain' if i < epochs / 2 else 'reward'
...
if __name__ == '__main__':
    ...
    for i in range(args.replicas):
        ...
        data.load('data/gdb9_9nodes.rewarddataset',
                  metric=METRIC, conditional_rate=args.cond_rate)
